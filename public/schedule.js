// public/schedule.js — COMPLETE FILE (string date compare; create / move / resize / delete works)
...
const toMin = t => {                    // accepts 0815 or 08:15 -> minutes
...
function draw() {
  ...
  sorted.forEach((w,row) => {
    ...
    for (let h=0; h<24; h++)
      grid.appendChild(make('',`grid-row:${row+2};grid-column:${h+2}`,'cell',{row,hour:h}));
...
const label = (t,r=1,c=1) => make(t,`grid-row:${r};grid-column:${c}`,'rowLabel');
const make  = (t,style,cls='',ds={}) => {
  const d = document.createElement('div');
  d.textContent=t; d.className=cls; d.style=style; Object.assign(d.dataset,ds); return d;
};
...
function doMove(e){ if(!mv) return;
  if(!mv.moved){ if(Math.abs(e.clientX-mv.startX)<4 && Math.abs(e.clientY-mv.startY)<4) return; mv.moved=true; mv.preview=mv.orig.cloneNode(true); mv.preview.style.opacity=.5; mv.preview.style.pointerEvents='none'; grid.appendChild(mv.preview);}  
...
grid.onmouseup = () => {
  ...
  openDlg('new',null,{row:dc.row,start:dc.start,end:dc.start+Math.round
