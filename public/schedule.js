...
function draw(){
  ...
  workers.forEach((w,row)=>{
    grid.appendChild(header(w.Name,row+2,1));              // names column
...
function cell(text,style,cls='',ds={}){
  const d=document.createElement('div');
  d.textContent=text; d.className=cls; d.style=style;
