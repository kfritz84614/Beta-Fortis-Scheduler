...
function draw(){
  ...
  sorted.forEach((w,row)=>{
    ...
    band.className='band'; band.style.gridRow=row+2; grid.appendChild(band);
  });

  shifts.filter(s=>sameDay(new Date(s.date),day))
...
function doMove(e){
  ...
  let st=Math.max(0,Math.min(1440-STEP,s.start+diffMin));
  let en=s.end+diffMin; if(en>1440){st-=en-1440; en=1440;}
...
async function endMove(e){
  ...
  const diffMin=Math.round((e.clientX-mv.startX)/pxPerMin/STEP)*STEP;
  const s=shifts[mv.idx];
  s.start=Math.max(0,Math.min(1440-STEP,s.start+diffMin));
  s.end  =Math.min(1440,Math.max(s.start+STEP,s.end+diffMin));
...
function openDlg(mode,idx,p){
  ...
  if(mode==='edit'){
    ...
    empIn.value=workers[p.row].Name;
