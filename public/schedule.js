...
function onResize(e){
  ...
  if (rs.side==='l')
    blk.start = Math.min(blk.end-STEP, Math.max(0, rs.orig.start + diff));
  else
    blk.end   = Math.max(blk.start+STEP, Math.min(1440, rs.orig.end + diff));
  draw();
}
function endResize(){
  rs = {};
...
function openDlg(mode,idx,preset){
  ...
  if (mode==='edit'){
    ...
    start.value = fmt(preset.start);
    end.value   = fmt(preset.end);
    notes.value = '';
...
f.onsubmit=e=>{
  ...
  if (f.index.value === '')
    ...
    Object.assign(blocks[+f.index.value],b);
  dlg.close(); draw();
};
