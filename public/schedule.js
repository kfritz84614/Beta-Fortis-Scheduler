...
/* ---------- persistence helpers ------------------------------- */
const saveShift=async s=>{const {id}=await fetch('/api/shifts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(s)}).then(r=>r.json());if(!s.id)s.id=id;};
const deleteShift=id=>fetch(`/api/shifts/${id}`,{method:'DELETE'});

/* ---------- util ---------------------------------------------- */
const firstStart=name=>{const x=shifts.filter(s=>s.name===name&&s.date===iso(day)).sort((a,b)=>a.start-b.start)[0];return x?x.start:1441};
const label=(t,r=1,c=1)=>{const d=document.createElement('div');d.className='rowLabel';d.textContent=t;d.style=`grid-row:${r};grid-column:${c}`;return d};
const cell=(r,c,ds={})=>{const d=document.createElement('div');d.className='cell';d.style=`grid-row:${r};grid-column:${c}`;Object.assign(d.dataset,ds);return d};
...
function draw(){
  ...
  sorted.forEach((w,r)=>{
    ...
    const band=document.createElement('div');band.className='band';band.style.gridRow=r+2;grid.appendChild(band);
  });
...
/* ---------- place block --------------------------------------- */
function placeBlock(s,idx,row){const band=grid.querySelectorAll('.band')[row];if(!band)return;const el=document.createElement('div');el.className='block';el.style.cssText=`left:${s.start/1440*100}%;width:${(s.end-s.start)/1440*100}%;background:${COLORS[s.role]||'#2563eb'}`;el.textContent=`${s.role} ${fmt(s.start)}-${fmt(s.end)}`;el.ondblclick=()=>openDlg('edit',idx);
  ['l','r'].forEach(side=>{const h=document.createElement('span');h.style=`position:absolute;${side==='l'?'left':'right'}:0;top:0;bottom:0;width:6px;cursor:ew-resize`;h.onmousedown=e=>startResize(e,idx,side);el.appendChild(h);});
...
async function endMove(){document.onmousemove=document.onmouseup=null;if(!mv)return;if(!mv.moved){openDlg('edit',mv.idx);mv=null;return;}const px=grid.querySelector('.band').getBoundingClientRect().width/1440;const diff=Math.round((event.clientX-mv.startX)/px/STEP)*STEP;const s=shifts[mv.idx];s.start=Math.max(0,Math.min(1440-STEP,s.start+diff));s.end=Math.min(1440,Math.max(s.start+STEP,s.end+diff));const diffRow=Math.round((event.clientY-mv.startY)/30);const newRow=Math.min(Math.max(0,mv.row+diffRow),workers.length-1);s.name=workers[newRow].Name;mv.preview.remove();mv=null;await saveShift(s);draw();}
...
delBtn.onclick = async () => {
  const i = +f.index.value;
  await deleteShift(shifts[i].id);
  shifts.splice(i, 1);
  dlg.close();
  draw();
};
...
f.onsubmit = async e => {
  ...
  if (f.index.value === '') shifts.push(s); else {
    s.id = shifts[+f.index.value].id;
    shifts[+f.index.value] = s;
  }
  await saveShift(s);
  dlg.close();
  draw();
